...
interface EnhancedTradeAnalyticsProps {
  userId?: string; // If provided, show analytics for specific user; if not, show for all users (admin view)
  isAdmin?: boolean;
}
...
const EnhancedTradeAnalytics: React.FC<EnhancedTradeAnalyticsProps> = ({ userId, isAdmin = false }) => {
      ...
      try {
        ...
        if (userId) {
          ...
          setTradeStats({
            ...
            avgRiskReward: 2.3,
            avgConfidence: 7.2,
            mostTradedPair: 'EURUSD',
            mostUsedStrategy: 'Breakout',
        ...
        setAiInsights([
          ...
          "Students are showing better risk management but still struggle with patience.",
        ...
        setAiSuggestions([
          ...
          "Focus on improving risk-reward ratios across all tiers.",
          "Introduce more practical exercises for identifying high-probability setups."
        ]);
      } catch (err) {
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {strategyData.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
          </select>
        </div>
      </div>
      ...
      {isAdmin && (
        <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 border border-blue-500/30 rounded-xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-lg">AI-Powered Class Insights</h3>
          </div>
...
export default EnhancedTradeAnalytics;